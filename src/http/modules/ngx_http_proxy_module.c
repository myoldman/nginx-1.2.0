...
//	ngx_uint_t i;
	...
	if(strlen(r->app_id) != 0) {
		char app_ip[32] = {0};
		strncpy(app_ip, (char *)r->connection->addr_text.data, r->connection->addr_text.len);
		if(!ngx_emp_server_check_appid_ip(r->app_id, app_ip)) {
			return NGX_HTTP_INTERNAL_SERVER_ERROR;
		}
	}
	...
	if(strlen(r->app_id) != 0 && strlen(r->access_token) != 0 && ngx_emp_server_api_verify_on()) {
		...
		memcpy(api_verify_t.time_local, ngx_cached_http_log_time.data,ngx_cached_http_log_time.len);
